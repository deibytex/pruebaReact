...
export const LogTable: React.FC<Props> = () => {
    ...
    const [fi, setFi] = useState<string>(moment().add(-1, 'month').format("YYYY-MM-DD"));
    const [ff, setFF] = useState<string>(moment().format("YYYY-MM-DD"));
    ...
    useEffect(() => {
        ...
        ConsultarUsuarios(contenedor).then(
            (response) => {
                ...
                // consultamos los datos al momento de traer los usuarios
                ConsultarLog();
            }
        ).catch((error) => {
    ...
    const restaFechas = (f1: string, f2: string) => {
        let fechainicial = moment(f1).format("YYYY-MM-DD");
        let fechafinal = moment(f2).format("YYYY-MM-DD");
            ...
            <div className="card card-rounded bg-transparent " style={{ width: '100%' }}  >
                <div className="row  col-sm-12 col-md-12 col-xs-12 rounded shadow-sm mt-2 bg-secondary  text-primary" style={{ width: '100%' }} >
                    <div className="col-sm-2 col-md-2 col-xs-2">
                        ...
                        <Form.Control className=" mb-3 " value={fi} type="date" name="fechaini" placeholder="Seleccione una fecha inicial" onChange={(e) => {
                            // buscamos el objeto completo para tenerlo en el sistema
                            setFi(e.currentTarget.value);
                        }} />
                    </div>
                    <div className="col-sm-2 col-md-2 col-xs-2">
                        <label className="control-label label label-sm" style={{ fontWeight: 'bold' }}>Fecha final</label>
                        <Form.Control className=" mb-3 " value={ff} type="date" name="fechaifin" placeholder="Seleccione una fecha final" onChange={(e) => {
                            // buscamos el objeto completo para tenerlo en el sistema
                            setFF(e.currentTarget.value);
                        }} />
                ...
                <div className="row col-sm-12 col-md-12 col-xs-12 py-2 bg-secondary text-primary">
                    <div className="col-sm-4 col-md-4 col-xs-4 border rounded shadow-sm" style={{ textAlign: 'center' }}>
                        <label className="control-label label label-sm " style={{ fontWeight: 'bold', textAlign: 'center' }}>Cargados</label>
                    ...
                    <div className="col-sm-4 col-md-4 col-xs-4 border rounded shadow-sm" style={{ textAlign: 'center' }}>
                        <label className="control-label label label-sm " style={{ fontWeight: 'bold', textAlign: 'center' }}>Modificados</label>
                        <div className="card text-black mb-3" style={{ marginTop: '5px', textAlign: 'center', flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', height: '100px' }}>
                            <span style={{ fontWeight: 'bold', fontSize: '30px' }}>{mmodificado}</span>
                        </div>
                    </div>
                    <div className="col-sm-4 col-md-4 col-xs-4 border rounded shadow-sm" style={{ textAlign: 'center' }}>
                        <label className="control-label label label-sm " style={{ fontWeight: 'bold', textAlign: 'center' }}>Descargados</label>
                        <div className="card text-black mb-3" style={{ marginTop: '5px', textAlign: 'center', flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', height: '100px' }}>
