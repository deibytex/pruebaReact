...
export const ExportarExcel : React.FC<Props>= ({NombreArchivo}) =>{
    const { ClienteSeleccionado, setClientes, Clientes, setClienteSeleccionado, dataTable, IsFiltrado, dataTableFiltrada} = useDataEventoCarga()
    const fileType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;carset=UTF-8';
    const fileExtension = '.XLSX';
    const exportarExcel = async() =>{
        if(IsFiltrado){
            if(dataTableFiltrada.length >0){
                let finaldata = dataTableFiltrada.map((item:any) =>{
                    let Objeto = {};
